...
The tmux status line modules are set as variables and prefixed with `@sonokai_status_<module>`.
...
set -g status-right "#{E:@sonokai_status_application}#{E:@sonokai_status_session}"
set -g status-left ""
```
...
```sh
set -g @sonokai_[module_name]_icon "icon"
```

### Override the specific module color

```sh
set -g @sonokai_[module_name]_color "color"
```

### Override the specific module text

```sh
set -g @sonokai_[module_name]_text "text"
```
...
```sh
set -g @sonokai_[module_name]_[option] ""
```
...
```sh
set -g @sonokai_date_time_icon ""
```

### Notes for TPM users

Make sure you load the sonokai theme prior to setting the status-left and/or
status-* options. This ensures the sonokai options (such as colors and
...
```bash
# load sonokai theme ...
run '~/.config/tmux/plugins/tmux/sonokai.tmux' # or where this file is located on your machine

# ... and then set status-left & status-right ...
set -g status-left "#{E:@sonokai_status_session}"

set -g status-right "#{E:@sonokai_status_[module_name]}"
set -ag status-right "#{E:@sonokai_status_[module_name]}"
set -agF status-right "#{E:@sonokai_status_[module_name]}"
...
set -agF status-right "#{E:@sonokai_status_weather}"
...
set -agF status-right "#{E:@sonokai_status_clima}"
...
set -agF status-right "#{E:@sonokai_status_load}"
```
...
set -agF status-right "#{@sonokai_status_gitmux}"
```
...
Add the following to your `~/.gitmux.conf` so that it uses sonokai colors:
...
set -agF status-right "#{E:@sonokai_status_pomodoro_plus}"
...
set -agF status-right "#{E:@sonokai_status_kube}"
