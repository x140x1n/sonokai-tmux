...
```bash
set -g @sonokai_window_text "#W"
set -g @sonokai_window_current_text "#W"
```
...
1. Ensure every `@sonokai.*` option is being set _before_
   the plugin is loaded. Loading occurs where you have `run` in your config file.
1. Double check for typos. There are two "c"s and two "p"s in "sonokai".
